...
## 2024-05-18 - Atomic Deletion for User Accounts
**Learning:** The previous implementation of `admin_delete_user` used a sequential `SELECT` via `get_user_by_email` followed by `session.delete()`. While functional, it forced 2 database roundtrips per deletion. For simple database deletions where the object data is not needed prior to deletion, using a direct `sqlalchemy.delete` query (e.g. `session.execute(delete(...))`) safely handles this pattern in a single query, cutting database roundtrips in half.
**Action:** When deleting records based on specific constraints and the object's data is not needed for application logic, prefer direct `DELETE` queries over `session.get()` or a specific SELECT followed by `session.delete()`.
