...
**Sync semantics (Tauri and server):** The server is **authoritative** for the tree after you bind a folder; deletes and uploads propagate per the engine. **Conflicts:** same file edited on two devices without syncing in between → **last successful upload wins**; there is **no** automatic conflicted copy or merge UI. Network and proxy limits (timeouts, max body) are described in [Network limitations](../network/limitations.md). Architecture rationale: [ADR 006 — Sync & trust boundaries](../adrs/006-sync-semantics-trust-boundaries.md).
